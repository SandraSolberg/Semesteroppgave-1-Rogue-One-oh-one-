...
public class Spider implements IActor {
	...
	public void doTurn(IGame game) {
		...
		if(attackIfPossible(game))return;

		else if (game.canGo(lastDir)) {
			     game.move(lastDir);
	...
	public boolean attackIfPossible(IGame game) {


		for (GridDirection dir : GridDirection.EIGHT_DIRECTIONS) {
			ILocation next = game.getLocation(dir);
			if(game.containsActor(next, Player.class)){
				game.displayMessage( getLongName() + " SAYS: HAHA GOT YOU PLAYER!");
				game.attack(dir);
				return true;
			}
